...
export const supabaseService = {
  ...
  async getHotelInfo(): Promise<HotelInfo> {
    ...
    try {
      ...
      if (err instanceof TypeError || String(err).includes("Failed to fetch") || String(err).includes("NetworkError")) {
        ...
        console.warn("Supabase network offline or database paused. Switching to local hotel_info fallback.");
      } else {
  ...
  async getRooms(): Promise<Room[]> {
    ...
    try {
      ...
      if (err instanceof TypeError || String(err).includes("Failed to fetch") || String(err).includes("NetworkError")) {
        connectionFailed = true;
        console.warn("Supabase network offline or database paused. Switching to local rooms fallback.");
      } else {
  ...
  async getBookings(): Promise<Booking[]> {
    ...
    try {
      ...
      if (err instanceof TypeError || String(err).includes("Failed to fetch") || String(err).includes("NetworkError")) {
        connectionFailed = true;
        console.warn("Supabase network offline or database paused. Switching to local bookings fallback.");
      } else {
  ...
  async getGallery(): Promise<GalleryImage[]> {
    ...
    try {
      ...
      if (err instanceof TypeError || String(err).includes("Failed to fetch") || String(err).includes("NetworkError")) {
        connectionFailed = true;
        console.warn("Supabase network offline or database paused. Switching to local gallery fallback.");
      } else {
  ...
  async getTestimonials(): Promise<Testimonial[]> {
    ...
    try {
      ...
      if (err instanceof TypeError || String(err).includes("Failed to fetch") || String(err).includes("NetworkError")) {
        connectionFailed = true;
        console.warn("Supabase network offline or database paused. Switching to local testimonials fallback.");
      } else {
